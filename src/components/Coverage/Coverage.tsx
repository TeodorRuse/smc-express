...
/** Coverage — cinematic band with dot-matrix Europe map and overlaid text. */
export function Coverage({ t }: { t: Translations }) {
  const [coverageRef, coverageInView] = useInView(0.05);

  return (
    <section className="coverage-band" id="coverage" ref={coverageRef as React.RefObject<HTMLElement>}>
      {/* Map fills the entire band */}
      <svg
        className={`coverage-band__map ${coverageInView ? 'coverage-band__map--visible' : ''}`}
        viewBox="-60 -40 600 460"
      ...
      <div className="coverage-band__fade" />

      {/* Text content */}
      <div className={`coverage-band__content ${coverageInView ? 'ai' : ''}`}>
